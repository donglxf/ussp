...
//			}
			...
			log.info("--------跟踪stream close 错误--------6");
			//app不为空，判断是否为外部系统用户
			Boolean flag = roleClient.isOS(app);
			log.info("--------跟踪stream close 错误--------7");
			if(flag==true) {
				log.info("--------跟踪stream close 错误--------8");
				//如果是外部系统，下一过滤器不执行
