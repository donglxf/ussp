...
var loadUserRoleListUrl=basepath + 'user/loadListByPage.json'; //列出用户所有角色列表
...
layui.use(['form', 'ztree', 'table'], function () {
    ...
    table.render({
        ...
        , cols: [[
            ...
            , {fixed: 'right', width: 178, title: '操作', align: 'center', toolbar: '#userrole_user_datatable_bar'}
        ]]
    });
    //监听操作栏
    ...
    table.on('tool(userrole_role_datatable_bar)', function (obj) {
        var data = obj.data;
        if (obj.event === 'getRole') { //禁用、删除
