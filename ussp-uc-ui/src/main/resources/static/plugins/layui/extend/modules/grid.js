layui.define(['form', 'laypage', 'laytpl'], function (exports) {
    var $ = layui.jquery
        ...
        , index: layui.grid ? (layui.grid.index + 10000) : 0
        , createNew: function (option) {
            var that = this;
            that.index = ++Grid.index;
            if (option.height && /^full-\d+$/.test(option.height)) {
            ...
            var grid = {
                ...
                checkBoxClick: option.checkBoxClick,
                rowid: option.rowid,
            ...
            var events = {
                ...
                , checkboxClick: function () {
                    var that = $(this);
                    if (grid.checkBoxClick)
                        grid.checkBoxClick(grid.pageData.rows[that.index()]);
                }
                //排序
                , order: function () {
                    var that = $(this);
            ...
            function fnSuccess() {
                grid.elem.html('');
                grid.elem.attr("lay-filter", "LAY-grid-" + that.index);
            ...
            function renderForm(type) {
                form.render(type, 'LAY-grid-' + that.index);
            }

            grid.renderForm = function (type) {
                renderForm(type);
            }
            ...
            function fnBody(html, content) {
                ...
                if (trs.length > 0) {
                    ...
                    // trs.on('click', events.rowClick);//注释行点击事件
                    //checkebox点击事件
                    trs.on('click', 'div.layui-form-checkbox', events.checkboxClick);
                }
            ...
            function rowsDom(rows, op) {
                ...
                //trs.on('click', events.rowClick); by tanrq
                //verify(trs);
                return trs;
            }
            ...
            grid.resize = function () {
                ...
                if (that.fullHeightGap) {
                    ...
                    grid.elem.find('.grid-body').css('height', height);
                }
