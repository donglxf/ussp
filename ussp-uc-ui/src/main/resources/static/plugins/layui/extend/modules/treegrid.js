                ...
                treeDom: function (trs, rows) {
                    ...
                    $.each(trs, function (i, tr) {
                        ...
                        if (!grid.singleSelect) {
                            tr.on('click', 'div.layui-form-checkbox', function () {
                                var $tr = $(this).parents("tr");
                                var selected = $(this).hasClass('layui-form-checked');
                                private_childSelect($(this), $tr, selected);
                                var _pid = HELPER.nodePid($tr);
                                if (_pid != null)
                                    private_parentSelect($tr, selected, _pid);
                            });
                        }
                    });

                    function private_childSelect($checkbox, $tr, selected) {
                        $tr.nextAll().each(function (_, tr) {
                            ...
                            if ($(tr).find('td input:checkbox').length > 0) {
                                $(tr).find('td input:checkbox').prop("checked", selected);
                            }
                        });
                        //判断点击的是否是首列的checkbox，如果是首列选中，则子项全部选中
                        if ($checkbox.parent().find(':checkbox').is($tr.find('td:first :checkbox'))) {
                            $tr.find('td input:checkbox').prop("checked", selected);
                        }
                        grid.renderForm('checkbox');
                    }

                    function private_parentSelect(tr, selected, pid) {
                        var trParent = grid.elem.find('tr.treegrid-' + pid);
                        if (trParent.length > 0) {
                            if (selected) {
                                if (trParent.find('td:first :checkbox').length > 0) {
                                    trParent.find('td:first :checkbox')[0].checked = true;
                                    tr.find('td:first :checkbox')[0].checked = true;
                                }
                            }
                            else {
                                // var $thisChecked = tr.find('input:checkbox:checked');
                                // if ($thisChecked.length == 1 && tr.find('td:first :checkbox')[0].checked) {//如果只有一个子项选中，并且为首列，则取消选中
                                //     //tr.find('td:first :checkbox')[0].checked = false;
                                // }
                                var $parentChecked = trParent.nextAll('tr.treegrid-parent-' + pid).find('input:checkbox:checked');
                                if ($parentChecked.length == 0) {//如果没有选中的子项，则父节点取消选中
                                    if (trParent.find('td:first :checkbox').length > 0)
                                        trParent.find('td:first :checkbox')[0].checked = false;
                                }
                            }
                            pid = HELPER.nodePid(trParent);
                            if (pid != null)
                                private_parentSelect(tr, selected, pid);
                        }
                        grid.renderForm('checkbox');
                    }
