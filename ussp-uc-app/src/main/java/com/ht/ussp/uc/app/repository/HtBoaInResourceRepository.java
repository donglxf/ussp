...
public interface HtBoaInResourceRepository extends JpaRepository<HtBoaInResource, Long>{
	...
	@Query(value="select new com.ht.ussp.uc.app.vo.ResVo(resCode,resNameCn,sequence,resType,resParent,fontIcon,resContent) from HtBoaInResource where status=0 and app= :app and resType in(:res_type)")
	public List<ResVo> queryResForY(@Param("res_type") List<String> res_type,@Param("app") String app);

	//查询资源--非管理员
	@Query(value="select new com.ht.ussp.uc.app.vo.ResVo(resCode,resNameCn,sequence,resType,resParent,fontIcon,resContent) from  HtBoaInResource  where status=0 and app= :app and resType in(:res_type) and resCode in(:res_code)")
