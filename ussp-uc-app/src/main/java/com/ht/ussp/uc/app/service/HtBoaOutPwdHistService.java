...
import com.ht.ussp.uc.app.repository.HtBoaOutPwdHistRepository;
...
public class HtBoaOutPwdHistService {
    ...
    public Object findAllByPage(PageConf pageConf, Object object) {
        if (null != object && object instanceof Map) {
            ...
            if (null != map.get("userId")) {
                ...
                HtBoaOutPwdHist u = new HtBoaOutPwdHist();
                u.setUserId(userId);
                ExampleMatcher matcher = ExampleMatcher.matching();
                Example<HtBoaOutPwdHist> ex = Example.of(u, matcher);
                if (null != pageable)
                    return this.htBoaOutPwdHistRepository.findAll(ex, pageable);
                else if (null != sort)
                    return this.htBoaOutPwdHistRepository.findAll(ex, sort);
                else
                    return this.htBoaOutPwdHistRepository.findAll(ex);
            }
        }
    ...
    public HtBoaOutPwdHist add(HtBoaOutPwdHist u) {
        return this.htBoaOutPwdHistRepository.saveAndFlush(u);
    }

    public HtBoaOutPwdHist update(HtBoaOutPwdHist u) {
